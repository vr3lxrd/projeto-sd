...
-- X	=>  Location: PIN_C9,	 I/O Standard: 2.5 V,	 Current Strength: Default
...
-- Equation(s):
-- \Selector0~0_combout\ = (\X~input_o\) # (\state.C~q\)
...
-- Equation(s):
-- \next_state.B~0_combout\ = (\X~input_o\ & ((\state.E~q\) # (!\state.A~q\)))
...
-- Equation(s):
-- \next_state.C~0_combout\ = (\X~input_o\ & ((\state.C~q\) # (\state.B~q\)))
...
-- Equation(s):
-- \next_state.D~0_combout\ = (!\X~input_o\ & \state.C~q\)
...
-- Equation(s):
-- \next_state.E~0_combout\ = (\X~input_o\ & \state.D~q\)
